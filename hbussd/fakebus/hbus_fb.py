...
from ..hbus.base import HbusDeviceAddress, HbusInstruction, HbusOperation
...
CONFIG_DATA_TYPE_INFO = {
    "h": HbusObjDataType.dataTypeByteHex,
...
class FakeBusDevice(HbusDevice):
    ...
    def create_read_response(self, objnum):
        ...
        if objnum == 0:
            ...
            object_read = [0, self.hbusSlaveObjects[objnum].size]
...
class FakeBusSerialPort(Protocol):
        ...
        for byte in data:
            ...
            if self.rxState == HbusRXState.SBID:
                ...
                if byte in HBUS_SCMDBYTELIST:
                    ...
                    byte == HBUSCOMMAND_SOFTRESET.cmd_byte
                ):  # softreset is different, doesnt specify addr field
                ...
                self.dataBuffer.append(byte)
                if (
                    self.dataBuffer[4] == HBUSCOMMAND_STREAMW.cmd_byte
                    or self.dataBuffer[4] == HBUSCOMMAND_STREAMR.cmd_byte
                ):
                    self.rxState = HbusRXState.STP
                else:
                    if byte > 0:
                ...
                if len(self.dataBuffer) <= (count + self.lastParamSize):
                    ...
                    if byte == 0xFF:
                        ...
                        self.rxState = HbusRXState.SBID
                        self.logger.debug(
                            "ignored malformed packet from master @PRM"
                        )
                        self.logger.debug(
                            "info: expected to have received {} bytes".format(
                                count + self.lastParamSize
                            )
                        )
                        self.logger.debug(
                            "packet size %d, dump: %s",
                            len(self.dataBuffer),
                            [hex(ord(x)) for x in self.dataBuffer],
                        )
                        self.dataBuffer = []
                        return
                ...
                if byte == 0xFF:
                    ...
                    self.rxState = HbusRXState.SBID
                    self.logger.debug(
                        "ignored malformed packet from master @STP"
                    )
                    self.logger.debug(
                        "packet size %d dump: %s",
                        len(self.dataBuffer),
        ...
        if self.busState == HbusBusState.LOCKED_OTHER:
            ...
            if self.addressingDevice is None:
                if (
                    ...
                    and packet[5] == 0
                    and self.deviceList[self.addressingDevice].deviceStatus
                    ...
                    ].create_read_response(0)

                    self.send_packet(
                        HBUSCOMMAND_RESPONSE,
                        FAKEBUS_MASTER_ADDRESS,
                        HbusDeviceAddress(0, 255),
                        params,
                    )
                    self.deviceList[
                        self.addressingDevice
                    ].deviceStatus = FakeBusDeviceStatus.deviceAddressing3
                    return

                elif (
                    packet[4] == HBUSCOMMAND_SEARCH.cmd_byte
                    or packet[4] == HBUSCOMMAND_KEYSET.cmd_byte
                    and self.deviceList[self.addressingDevice].deviceStatus
                    == FakeBusDeviceStatus.deviceAddressing3
                ):
            ...
            if packet[4] == HBUSCOMMAND_SEARCH.cmd_byte:
                ...
                for device in list(self.deviceList.values()):
                    if device.deviceStatus == FakeBusDeviceStatus.deviceIdle:
                        # start addressing
                        device.deviceStatus = (
                            FakeBusDeviceStatus.deviceAddressing1
                        )
                        # queue for addressing
                        self.addressingQueue.appendleft(
    ...
    def address_next_dev(self):
        ...
        if (
            ...
            self.deviceList[
                self.addressingDevice
            ].deviceStatus = FakeBusDeviceStatus.deviceAddressing2
            self.send_packet(
                HBUSCOMMAND_BUSLOCK,
                FAKEBUS_MASTER_ADDRESS,
                HbusDeviceAddress(0, 255),
            ...
            self.deviceList[
                self.addressingDevice
            ].deviceStatus = FakeBusDeviceStatus.deviceEnumerated
            self.addressingDevice = None
    ...
    def build_bus(self):
        ...
        devpath = os.path.join(
            self.config_path, self.config.get("fakebus", "object_dir")
        )
        ...
        for devfile in devfiles:
            ...
            # UID
            device.hbusSlaveUniqueDeviceInfo = int(
                devconf.get("device", "uid"), 16
            )
            device.hbusSlaveDescription = devconf.get("device", "descr")
            device.hbusSlaveObjectCount = devconf.getint(
                "device", "object_count"
            )
            device.hbusSlaveEndpointCount = devconf.getint(
                "device", "endpoint_count"
            )
            device.hbusSlaveInterruptCount = devconf.getint(
                "device", "int_count"
            )
            ...
            for section in devconf.sections():
                ...
                try:
                    ...
                    if (
                        obj.objectDataType
                        != HbusObjDataType.dataTypeFixedPoint
                    ):
                        obj.objectDataTypeInfo = CONFIG_DATA_TYPE_INFO[
                            data_type_info
                        ]
                    else:
                ...
                obj.last_value = self.list_val_to_int(
                    raw_value, obj.objectDataType
                )
            ...
            self.logger.debug(
                'fake device "'
                + device.hbusSlaveDescription
                + '" <'
                + hex(device.hbusSlaveUniqueDeviceInfo)
                + "> added"
            )
